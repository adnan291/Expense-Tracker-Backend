...
async function saveToDatabase(event) {  
    ...
    const res =  await axios.post("http://3.7.248.136:4000/expense/add-expense",obj,{headers:{'Authorization':token}});
    ...
    async function fetchFilteredData(timePeriod) {
      try {
        ...
        const res = await axios.post(
          'http://3.7.248.136:4000/expense/get-filtered-expenses',
          data,
...
// console.log("Description" ,expense.description);
   ...
   function showLeaderboard() {
    ...
    inputElement.onclick = async () => {
        try {
            const token = localStorage.getItem('token');
            const userLeaderBoardArray = await axios.get('http://3.7.248.136:4000/premium/showLeaderboard', {
                headers: { Authorization: token }
            });
   ...
   function showDownloaded() {
    ...
    inputElement.onclick = async () => {
        try {
            const token = localStorage.getItem('token');
            const userDownloadedArray = await axios.get('http://3.7.248.136:4000/premium/showDownloaded', {
                headers: { Authorization: token }
            });
  ...
  document.getElementById('downloadBtn').onclick = async function () {
    await axios.get('http://3.7.248.136:4000/expense/download', {
      headers: { Authorization: token }
  }).then((response) => {
  ...
  async function getExpense(page, expPerPage) {
    try {
      ...
      const res = await axios.get(`http://3.7.248.136:4000/expense/get-expense?page=${page}&expPerPage=${expPerPage}`,{ headers: { Authorization: token } });
  ...
  async function deleteExpense(expenseId) {
    try{
         await axios.delete(`http://3.7.248.136:4000/expense/delete-expense/${expenseId}`,{headers:{'Authorization':token}});
    ...
    document.getElementById("goPremiumBtn").onclick = async function(e) {
      response = await axios.get('http://3.7.248.136:4000/purchase/premiumMembership', {headers:{'Authorization':token}})
      // console.log(response)
      var options = {
        ...
        async function (response) { 
          await axios.post("http://3.7.248.136:4000/purchase/updateTransactonStatus",{
